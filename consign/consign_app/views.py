...
from consign_app.models import Enquiry, UserLogin,Login,ImageUpload, AddConsignment,AddConsignmentTemp,Disel, AddTrack,FeedBack, Branch,Driver,Vehicle, Staff,Consignee, Consignor,TripSheetTemp,TripSheetPrem, Account,Expenses
...
def serviceappointment(request):
...
def login(request):
    if request.method == "POST":
        ...
        try:
            # Check if the user with the given username and password exists in the login table
...
def clear(request, id):
    try:
        ...
        # Delete the object
        enquiry.delete()
        # Redirect or render the appointment details page
        return redirect('appointmentdetails')  # Redirect to avoid issues with refresh/delete
    except Enquiry.DoesNotExist:
        # Handle the case where the Enquiry with the given id does not exist
        return render(request, 'appointmentdetails.html', {'error': 'Record not found'})

def upload_image(request):
    success = None
    if request.method == 'POST' and request.FILES:
        image_files = request.FILES.getlist('image')  # Get the list of uploaded images
        for img in image_files:
            new_image = ImageUpload(image=img)
            new_image.save()
        success = 'Images uploaded successfully!'

    return render(request, 'upload_image.html', {'success': success})
...
def delete_image_gallery(request, image_id):
    image = get_object_or_404(ImageUpload, id=image_id)
    image.delete()
    return redirect('images_view')  # Redirect to the gallery view
...
def adminView_Consignment(request):
    ...
    if request.method == 'POST':
        ...
        print("Filtered Consignments: {}".format(queryset))  # Debugging: Print the filtered queryset
